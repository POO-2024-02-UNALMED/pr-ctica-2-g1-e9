...
from gestorAplicacion.clasesDeAdministracion.Prestamo import Prestamofrom uiMain.FieldFrame import FieldFrame
...
def __init__(self, root, sistema):
    ...
    self.desc = """En esta opcion podras consultar la disponibilidad para prestamo de losdiferentes recursos de la biblioteca, usando \ncriterios como Sede, Titulo, Autor, Fecha. Para de esta manera generar un prestamo en la biblioteca. """
...
class busquedaBasica(Frame):
    ...
    self.botonLimpiar = Button(self, text="Limpiar campos", command= lambda:
    self.entrada.delete(0, 'end'), font=("Arial", 11))
...
def confirmarExistencia(self):
    ...
    if self.opciones.get() == "Libro":
        ...
        try:
            ...
            return
messagebox.showerror("Error",ComputadorNoEncontrado(nombre).getError())
    ...
    try:
        ...
        for biblioteca in self.sistema.get_bibliotecas():
            for copia in biblioteca.get_copias():
                if recurso.get_nombre() == copia.get_nombre() and biblioteca.get_nombre()
not in sedes:
        ...
        def realizarReserva():
            ...
            for copia in sedeSel.get_copias():
                if copia.get_nombre() == recurso.get_nombre():
                    copiaSel = copia self.sistema.get_user().get_prestamos().append(Prestamo(self.sistema.get_user(), copiaSel, "Particular", date.today(), fechaSeleccionada, sedeSel)) 
                        sedeSel.get_copias().remove(copiaSel)
                        messagebox.askokcancel(title="Reserva realizada", message="¡Su reservahasido realizada con exito! No olvide devolver su recurso :)")
                        self.kill(self)
        ...
        try:
            ...
            for biblioteca in self.sistema.get_bibliotecas():
                for pc in biblioteca.get_PCs():
                    if recurso.get_nombre() == pc.get_nombre() and biblioteca.get_nombre() not
in sedes:
                    sedes.append(biblioteca.get_nombre())
            if not sedes:
            ...
            def realizarReserva():
                ...
                messagebox.askokcancel(title="Reserva realizada", message="¡Su reservahasidorealizada con exito! No olvide devolver su recurso :)")
                self.kill(self)
...
class busquedaPorLista(Frame):
    ...
    def computadorOLibro(self, evento):
        if self.opciones.get() == "Libro":
            ...
            lista = Text(self, border=False, font=("Arial", 11), borderwidth=2,
            ...
            lista.grid(row=2, column=0, columnspan=2, pady=5)
            lista.delete("1.0", "end")
            listaComputadores = [computador.get_nombre() for computador in
            self.sistema.get_computadores()]
            lista.config(height=10)
    ...
    def reservarLibro(self):
        ...
        try:
            ...
            for biblioteca in self.sistema.get_bibliotecas():
                for copia in biblioteca.get_copias():
                    if self.libroSel.get_nombre() == copia.get_nombre() and
    biblioteca.get_nombre() not in sedes:
                sedes.append(biblioteca.get_nombre())
            if not sedes:
    ...
    def reservarComputador(self):
        ...
        try:
            if int(self.seleccion.getValue("ID: ")) < 0:
                raise IndexFuera
            self.computadorSel =
            self.sistema.get_computadores()[int(self.seleccion.getValue("ID: "))]
            sedes = []
            self.kill(self)
            for biblioteca in self.sistema.get_bibliotecas():
                for pc in biblioteca.get_PCs():
                    if self.computadorSel.get_nombre() == pc.get_nombre() and
            biblioteca.get_nombre() not in sedes:
                sedes.append(biblioteca.get_nombre())
            if not sedes:
